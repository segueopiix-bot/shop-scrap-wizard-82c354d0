...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('klivopay webhook payload:', JSON.stringify(body));

    const data = body?.data || body;
    const transactionId =
      data?.transaction_hash || data?.hash || data?.id || body?.transaction_hash || body?.hash;
    const status =
      data?.status || data?.payment_status || body?.status || body?.payment_status;
    ...
    const paidStatuses = ['paid', 'approved', 'authorized', 'completed', 'succeeded', 'confirmed'];
