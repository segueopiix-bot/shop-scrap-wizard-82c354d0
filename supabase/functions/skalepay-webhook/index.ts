...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('skalepay webhook payload:', JSON.stringify(body));

    // Skalepay envia: { type, data: { id, status, ... } } ou variações
    const data = body?.data || body;
    const transactionId =
      data?.hash || data?.id || data?.transactionId || data?.transaction ||
      body?.hash || body?.id || body?.transactionId;
    const status =
      data?.status || data?.paymentStatus || data?.payment_status ||
      body?.status || body?.paymentStatus || body?.payment_status;
