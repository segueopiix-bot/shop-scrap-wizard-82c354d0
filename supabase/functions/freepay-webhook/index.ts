...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('freepay webhook payload:', JSON.stringify(body));

    // FreePay envia as chaves em PascalCase (Id, Status, PaidAt) e em payloads
    // alternativos pode vir aninhado em data/transaction.
    const data = body?.data || body?.transaction || body;
    const transactionId =
      data?.Id || data?.id || data?.hash || data?.transactionId || data?.transaction_id ||
      body?.Id || body?.id || body?.transactionId;
    const status =
      data?.Status || data?.status || data?.paymentStatus || data?.payment_status ||
      body?.Status || body?.status || body?.paymentStatus || body?.payment_status;
    ...
    const paidStatuses = ['paid', 'approved', 'authorized', 'completed', 'succeeded'];
