...
async function sendPushcutApproved(order: any) {
  try {
    ...
    await fetch(PUSHCUT_APPROVED_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'PIX Aprovado', text: `R$ ${amount} • ${order?.customer_name || ''}` }),
    });
...
async function sendUtmifyPaid(order: any) {
  ...
  } catch (e) { console.error('Utmify paid failed:', e); }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('avenpayments webhook payload:', JSON.stringify(body));

    const data = body?.data || body?.payment || body;
    const transactionId =
      data?.id || data?.paymentId || data?.transactionId || data?.externalRef ||
      body?.id || body?.paymentId;
    const status =
      data?.status || data?.paymentStatus || body?.status || body?.event;
    ...
    const paidStatuses = ['paid', 'approved', 'authorized', 'completed', 'succeeded', 'confirmed', 'PAID', 'APPROVED'];
    const isPaid = paidStatuses.includes(String(status));
    ...
    if (isPaid) {
      ...
      await supabaseAdmin
        .from('orders')
        .update({ status: String(status || 'pending').toLowerCase() })
        .eq('transaction_id', String(transactionId));
    }
